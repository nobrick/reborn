defmodule Machine.Adapters.CloudForest.DataWriter do
  ...
  def handle_call({:save_data, path, chunks, _opts}, _from,
                  %{header_string: header, feature_keys: keys} = state) do
  ...
  defp build_chunk_output([%{d_la: d_la, id: id}|non_target_deltas] = _chunk,
                          feature_keys) do
    data =
      [id, d_la] ++
